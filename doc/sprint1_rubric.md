...
- Release Goal requires more elaboration. You should talk about what you are aiming to achieve.
Your Mark: 2
  ...
  - Sprint Planning meeting (sprintN.md) (max 2 marks)
    ...
    Note: It would have been good to mention under the task breakdown about which member is responsible for each task.
...
Your Mark: 1 (Some commits missing ticket number e.g june 9 commits, 1. Story Details basic UI, 2. updated StoryDetails state to match the database)
...
## System Design (PDF or MD file or another suitable format) (max 10 marks) 
  ...
  - CRC Cards [or equivalent, if the team is not using CRC) (max 4 marks)
    ...
    Your Mark: 3 (Collaborators for couple of CRC cards missing)
...
## Sprint Demo (Max 14 marks) 
  ...
  - Working software (max 8 marks)
    ...
    - Form fields width should be reduced  
    - Should consider security for username and password 
    - user info should be displayed once logged in 
  ...
  - Presentation (max 2 marks)
    ...
    - Should redirect the user to sign in form from sign up from if the user has the account already 
    1 
